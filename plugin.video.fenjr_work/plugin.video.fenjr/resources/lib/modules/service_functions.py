...
class CheckSettingsFile:
	def run(self):
		...
		profile_dir = translate_path('special://profile/addon_data/plugin.video.fenjr/')
		if not path_exists(profile_dir): kodi_utils.make_directorys(profile_dir)
		settings_xml = translate_path('special://profile/addon_data/plugin.video.fenjr/settings.xml')
...
class ReuseLanguageInvokerCheck:
	def run(self):
		...
		addon_xml = translate_path('special://home/addons/plugin.video.fenjr/addon.xml')
...
class AutoRun:
	def run(self):
		...
		if settings.auto_start_fen(): kodi_utils.execute_builtin('RunAddon(plugin.video.fenjr)')
