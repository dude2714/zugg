...
class Navigator:
	...
	def tools(self):
		...
		self.add({'mode': 'magneto_health_check', 'isFolder': 'false'}, tools_ins % 'Magneto Health Check', 'settings')
	...
	def genres(self):
		menu_type = self.params.get('menu_type')
	...
	def folder_navigator(self):
		...
		folder_path, setting_id = self.params.get('folder_path'), self.params.get('setting_id')
	...
	def build_shortcut_folder_list(self):
		...
		list_name = self.params.get('name')
	...
	def because_you_watched(self):
		media_type = self.params.get('menu_type')
	...
	def build_main_list(self):
		menu_items = currently_used_list(self.list_name) or []
		if not menu_items:
			try:
				nc.rebuild_database()
				menu_items = currently_used_list(self.list_name) or []
