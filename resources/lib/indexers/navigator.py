...
class navigator:
    def _has_download_items(self, path):
        if not path:
            return False
        try:
            return len(control.listDir(path)[0]) > 0
        except:
            return False

    def root(self):
        api_keys.chk()
        module_label = 'PromiseScrapers' if control.setting('external.scraper.module') == '0' else 'ViperScrapers'
        ...
        self.addDirectoryItem(32007, 'liveTvProviders', 'channels.png', 'DefaultMovies.png')
        ...
        self.addDirectoryItem('Current Scraper: %s' % module_label, 'openScraperSettings', 'iconT.png', 'DefaultAddonProgram.png', isFolder=False)
    ...
    def live_tv_providers(self):
        last_provider = control.setting('live.tv.last.provider') or 'all'
        provider_labels = {
            'all': 'All Providers',
            'google_tv': 'Google TV',
            'roku_tv': 'Roku TV',
        ...
        self.addDirectoryItem('Roku TV', 'channels&provider=roku_tv', 'tvshows.png', 'DefaultTVShows.png')
        self.addDirectoryItem('Tubi TV', 'channels&provider=tubi_tv', 'channels.png', 'DefaultMovies.png')
        self.addDirectoryItem('LG TV', 'channels&provider=lg_tv', 'iconT.png', 'DefaultAddonProgram.png')
        self.endDirectory()
    ...
    def tvshows(self, lite=False):
        ...
        self.addDirectoryItem('TV Show Mosts', 'tvshows&url=popular', 'featured.png', 'DefaultTVShows.png')
    ...
    def mytvshows(self, lite=False):
        try:
            if traktCredentials == False and imdbCredentials == False and providers.SCRAPER_INIT == False:
                self.addDirectoryItem('No Trakt or IMDb account configured', 'openSettings&query=0.0', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
                if lite == False:
    ...
    def tools(self):
        ...
        self.addDirectoryItem('Scraper Module: %s' % module_label, 'toggleScraperModule', 'iconT.png', 'DefaultAddonProgram.png', isFolder=False)
        self.addDirectoryItem('Open Current Scraper Settings', 'openScraperSettings', 'iconT.png', 'DefaultAddonProgram.png', isFolder=False)
    ...
    def downloads(self):
        ...
        if self._has_download_items(movie_downloads):
            self.addDirectoryItem(32001, movie_downloads, 'movies.png', 'DefaultMovies.png', isAction=False)
        if self._has_download_items(tv_downloads):
    ...
    def addDirectoryItem(self, name, query, thumb, icon, context=None, queue=False, isAction=True, isFolder=True):
        ...
        if artPath is not None:
            themed_thumb = os.path.join(artPath, thumb)
            thumb = themed_thumb if os.path.exists(themed_thumb) else icon
        else:
            thumb = icon
        cm = []
        ...
        if context is not None:
            ...
            for ctx in contexts:
                if not isinstance(ctx, (list, tuple)) or len(ctx) != 2:
                    continue
                try:
                    c_label = control.lang(ctx[0])
                except:
                    c_label = ctx[0]
                cm.append((c_label, 'RunPlugin(%s?action=%s)' % (sysaddon, ctx[1])))
