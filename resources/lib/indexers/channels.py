...
class channels:
    ...
    def _provider_channels(self, provider):
        all_channels = [
        ...
        provider_buckets = {
            'all': [
                'ActionWomen', 'ActionWomen HD', 'Christmas 24', 'Christmas 24+',
                'Film4', 'Film4 HD', 'Film4+', 'Horror Channel', 'Horror Channel+',
                'ROK', 'Sky Action', 'Sky Action HD', 'Sky Christmas', 'Sky Christmas HD',
                'Sky Comedy', 'Sky Comedy HD', 'Sky Family', 'Sky Family HD',
                'Sky Greats', 'Sky Greats HD', 'Sky Hits', 'Sky Hits HD',
                'Sky Premiere', 'Sky Premiere HD', 'Sky Premiere+', 'Sky ScFi/Horror',
                'Sky ScFi/Horror HD', 'Sky Thriller', 'Sky Thriller HD', 'Sony Action',
                'Sony Action+', 'Sony Christmas', 'Sony Christmas+', 'Sony Movies',
                'Sony Movies+', 'TalkingPictures', 'TCM Movies', 'TCM Movies+'
            ],
            'google_tv': ['Sky Premiere', 'Sky Premiere HD', 'Sky Greats', 'Sky Greats HD', 'Film4', 'Film4 HD', 'Sky Family', 'Sky Family HD'],
            'roku_tv': ['Sky Action', 'Sky Action HD', 'Sky Thriller', 'Sky Thriller HD', 'Sony Action', 'Sony Action+', 'Sky Hits', 'Sky Hits HD'],
            'tubi_tv': ['Horror Channel', 'Horror Channel+', 'Sky ScFi/Horror', 'Sky ScFi/Horror HD', 'ROK', 'TalkingPictures', 'TCM Movies', 'TCM Movies+'],
            'lg_tv': ['Sky Comedy', 'Sky Comedy HD', 'Film4+', 'Christmas 24', 'Christmas 24+', 'Sony Movies', 'Sony Movies+', 'Sony Christmas', 'Sony Christmas+']
        }

        names = provider_buckets.get(provider)
        if not names:
            return all_channels

        selected = [c for c in all_channels if c[0] in names]
        return selected or all_channels
    ...
    def get(self, provider=None):
        if provider:
            ...
            selected_provider = provider_labels.get(provider, provider)
            try:
                control.setSetting('live.tv.last.provider', provider)
            except:
                pass
            control.infoDialog('Live TV source: %s' % selected_provider, time=3000)

        channels = self._provider_channels(provider)
