...
def getTraktAsJson(url, post=None):
    try:
        ...
        if r is None:
            log_utils.log('getTraktAsJson - __getTrakt returned None for URL: %s' % url, 1)
            return None
        ...
        log_utils.log('getTraktAsJson Error for URL: %s - %s' % (url, str(e)), 1)
        pass
...
def authTrakt():
    try:
        ...
        # Read keys fresh from settings so newly entered values are picked up
        _client_id = control.setting('trakt.client_id').strip()
        _client_secret = control.setting('trakt.client_secret').strip()
        if not _client_id:
            _client_id = api_keys.trakt_client_id
        if not _client_secret:
            _client_secret = api_keys.trakt_secret

        result = getTraktAsJson('/oauth/device/code', {'client_id': _client_id})
        if result is None:
            control.infoDialog('[COLOR red]Trakt API Error[/COLOR]\n\nFailed to connect to Trakt.\nCheck your internet connection and try again.', time=5000)
            raise Exception()
        ...
        for i in range(0, expires_in):
            try:
                ...
                r = getTraktAsJson('/oauth/device/token', {'client_id': _client_id, 'client_secret': _client_secret, 'code': device_code})
        ...
        headers = {'Content-Type': 'application/json', 'trakt-api-key': _client_id, 'trakt-api-version': 2, 'Authorization': 'Bearer %s' % token}
