...
def routing(_argv):
    ...
    def _info(message, time=None):
        from resources.lib.modules import control
        if time is None:
            control.infoDialog(message, time=10000)
        else:
            control.infoDialog(message, time=time)
    ...
    if action == None:
        ...
        _info('Popular IMDb User Lists is disabled', time=4000)

    elif action == 'liveTvProviders':
        from resources.lib.indexers import navigator
        navigator.navigator().live_tv_providers()
        ...
        channels.channels().get(provider=provider)
        ...
        items_per_page = str(control.setting('items.per.page')) or '20'
        entries = [
        ...
        for label, metric, period in entries:
            url = 'https://api.trakt.tv/shows/%s/%s?limit=%s&page=1' % (metric, period, items_per_page)
            navigator.navigator().addDirectoryItem(label, 'tvshows&url=%s' % quote_plus(url), 'trakt.png', 'DefaultTVShows.png')
    ...
    elif action == 'toggleScraperModule':
        from resources.lib.modules import control
        current_module = control.setting('external.scraper.module') or '1'
        new_module = '0' if current_module == '1' else '1'
        control.setSetting('external.scraper.module', new_module)
        module_label = 'PromiseScrapers' if new_module == '0' else 'ViperScrapers'
        _info('Scraper module set to %s' % module_label, time=5000)
        control.refresh()

    elif action == 'promisescrapersettings':
        from resources.lib.modules import control
        control.setSetting('external.scraper.module', '0')
        ...
        if not _has(source):
            _info('No source available')
        else:
            try:
                item = json.loads(source)[0]
                resolved = sources.sources().sourcesResolve(item, info=True)
                downloader.download(name, image, resolved)
            except:
                _info('Download failed')
        ...
        if all((_has(name), _has(title), _has(year), _has(imdb))):
            libtools.libmovies().add(name, title, year, imdb)
        ...
        from resources.lib.modules import libtools
        if all((_has(tvshowtitle), _has(year), _has(imdb), _has(tmdb))):
