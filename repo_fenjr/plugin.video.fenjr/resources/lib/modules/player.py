...
poster_empty = kodi_utils.translate_path('special://home/addons/plugin.video.fenjr/resources/media/box_office.png')
fanart_empty = kodi_utils.translate_path('special://home/addons/plugin.video.fenjr/fanart.png')
	...
	def run(self, url=None, media_type=None):
		...
		try:
			...
			if library_item and not background:
				...
				logger('FEN PLAYBACK', 'play request media_type=%s title=%s season=%s episode=%s' % (self.media_type, self.title, self.season, self.episode))
	...
	def monitor(self):
		...
		while not self.isPlayingVideo():
			...
			if time.time() - start_time >= startup_timeout:
				logger('FEN PLAYBACK', 'startup timeout media_type=%s title=%s season=%s episode=%s' % (self.media_type, self.title, self.season, self.episode))
				kodi_utils.notification('Fen Jr playback did not start. Check kodi.log for FEN PLAY entries.', 4000)
				return
			kodi_utils.sleep(100)
		logger('FEN PLAYBACK', 'playback started media_type=%s title=%s season=%s episode=%s' % (self.media_type, self.title, self.season, self.episode))
		kodi_utils.close_all_dialog()
