...
class Sources():
	...
	def get_sources(self):
		...
		if not results:
			...
			try:
				...
				logger('FEN get_sources process_results Exception', str(e))
				results = self.orig_results[:]
	...
	def filter_results(self, results):
		results = [i for i in results if i.get('quality', 'SD') in self.quality_filter]
		if not self.include_3D_results: results = [i for i in results if not '3D' in i.get('extraInfo', '')]
		if self.filter_size:
			...
			max_size = string_to_float(get_setting('results.size.auto', '10000'), '10000') / 1000
			if self.include_unknown_size:
				results = [i for i in results if i.get('scrape_provider', '').startswith('folder') or i.get('size', 0) <= max_size]
			else:
				results = [i for i in results if i.get('scrape_provider', '').startswith('folder') or 0.01 < i.get('size', 0) <= max_size]
		return results

	def sort_results(self, results):
		def _add_keys(item):
			provider = item.get('scrape_provider', 'external')
			if provider == 'external':
				debrid_name = item.get('debrid') or item.get('cache_provider', '')
				if debrid_name.startswith('Uncached '): debrid_name = debrid_name.replace('Uncached ', '')
				account_type = debrid_name.lower() if debrid_name else 'real-debrid'
	...
	def activate_external_providers(self):
		if not self.debrid_torrent_enabled and not self.debrid_hoster_enabled:
			...
			for addon_id in ('script.module.fenomscrapers', 'script.module.cocoscrapers', 'script.module.viperscrapers', 'script.module.magneto'):
				ensure_external_lib_path(addon_id)
			package_choices = [settings.external_scraper_package(), 'fenomscrapers', 'cocoscrapers', 'viperscrapers', 'magneto']
			seen = []
			...
			if not external_sources:
				notification('No scraper module found. Install FenomScrapers, CocoScrapers, ViperScrapers, or Magneto.', 4000)
				self.active_external = False
				return
		...
		for item in results:
			...
			if cache_provider:
				if cache_provider in debrid_names:
					cache_provider = '[COLOR %s]%s[/COLOR]' % (item_color, cache_provider)
				provider_display = '%s | %s' % (provider, cache_provider)
			elif debrid_provider:
				if debrid_provider in debrid_names:
					debrid_provider = '[COLOR %s]%s[/COLOR]' % (item_color, debrid_provider)
				provider_display = '%s | %s' % (provider, debrid_provider)
			else:
				provider_display = provider
			if size_label:
				size_label = '[COLOR %s]%s[/COLOR]' % (item_color, size_label)
				label = '[%s] %s | %s | %s' % (quality, provider_display, size_label, name)
			else:
	...
	def _get_quality_rank(self, quality):
		return quality_ranks.get(quality, 4)
	...
	def _size_value(self, item):
		try: return float(item.get('size', 0) or 0)
		except: return 0

	def _format_size_label(self, item):
		size_label = item.get('size_label')
		if size_label: return size_label
		size_value = self._size_value(item)
		if size_value > 0: return '%.2f GB' % size_value
		return None

	def _debrid_top(self, items):
		def _rd_quality_bucket(item):
			q = item.get('quality', 'SD')
			if q == '4K': return 0
			if q in ('1080p', '1440p'): return 1
			if q in ('720p', 'HD'): return 2
			if q in ('CAM', 'SCR', 'TELE'): return 4
			return 3  # SD
		def _sort_rd(results):
		...
		other_debrid = _sort_by_size([i for i in items if i.get('debrid') in debrid_names and i not in rd_cached and i not in other_cached and i not in rd_hoster])
	...
	def furkPacks(self, name, file_id, highlight=None, download=False):
		...
		default_furk_icon = translate_path('special://home/addons/plugin.video.fenjr/resources/media/furk.png')
	...
	def debridPacks(self, debrid_provider, name, magnet_url, info_hash, highlight=None, download=False):
		...
		default_debrid_icon = translate_path('special://home/addons/plugin.video.fenjr/resources/media/%s' % icon)
	...
	def play_file(self, results, source={}, autoplay=False, background=False):
		resolve_retry_pause_ms = 1800
		def _source_label(item):
			provider = item.get('provider', item.get('scrape_provider', 'unknown'))
			cache_provider = item.get('cache_provider', '')
			debrid_provider = item.get('debrid', '')
			source_name = item.get('name') or item.get('URLName') or item.get('source', 'stream')
			return 'provider=%s cache=%s debrid=%s source=%s name=%s' % (provider, cache_provider, debrid_provider, item.get('source', ''), source_name)
		def _url_summary(url):
			if not url:
				return 'none'
			try:
				stripped = str(url).split('|')[0]
		...
		def _resolve_dialog():
			...
			try:
				for count, item in enumerate(items, 1):
					try:
						if count > 1:
							# Give each candidate a bit more time before moving to the next one.
							sleep(resolve_retry_pause_ms)
						...
						logger('FEN PLAY RESOLVE', 'attempt=%s/%s %s' % (count, total_items, _source_label(item)))
						url = self.resolve_sources(item, self.meta)
						if url == 'uncached':
							url = _uncached_confirm(item)
							if url is None: return
						if url:
							logger('FEN PLAY RESOLVE', 'resolved %s url=%s' % (_source_label(item), _url_summary(url)))
							self.url = url
							break
						logger('FEN PLAY RESOLVE', 'failed %s' % _source_label(item))
					except Exception as e:
		...
		try:
			self._kill_progress_dialog()
			if autoplay:
				cached_items = [i for i in results if not 'Uncached' in i.get('cache_provider', '')]
				items = self._debrid_top(cached_items)
			...
			if not self.url:
				logger('FEN PLAY', 'no playable url returned after %s attempts' % total_items)
				notification(32760, 2000)
				return None
			logger('FEN PLAY', 'starting playback url=%s' % _url_summary(self.url))
		...
		try:
			if 'cache_provider' in item:
				...
				if 'Uncached' in cache_provider:
					# Do not send uncached links into cloud caching. Try direct resolve paths only.
					url = resolve_cached_torrents(item['debrid'], item['url'], item['hash'], title, season, episode)
					if url: return url
					if item.get('debrid') in ('Real-Debrid', 'Premiumize.me', 'AllDebrid', 'TorBox') and not item.get('source', '').lower() == 'torrent':
						url = resolve_debrid(item['debrid'], item.get('provider', ''), item['url'])
						if url: return url
