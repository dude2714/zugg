...
def build_navigate_to_page(params):
	...
	icon = kodi_utils.translate_path('special://home/addons/plugin.video.fenjr/resources/media/item_jump.png')
...
def open_settings(query, addon='plugin.video.fenjr'):
...
#	addon_ids = ('plugin.video.fenjr', 'script.module.fenomscrapers', 'script.module.myaccounts')
	from modules.settings import external_scraper_module
	external_addon = external_scraper_module()
	addon_ids = ('plugin.video.fenjr', external_addon)
...
def sync_MyAccounts(silent=False):
	...
	try:
		...
		if not tmdb_key: tmdb_key = 'a07324c669cac4d96789197134ce272b'
...
def toggle_language_invoker():
	...
	kodi_utils.close_all_dialog()
	kodi_utils.sleep(100)
	addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.fenjr/addon.xml')
...
def refresh_artwork():
	...
	for item in ('icon.png', 'fanart.png'):
		try:
			icon_path = kodi_utils.translate_path('special://home/addons/plugin.video.fenjr/%s' % item)
