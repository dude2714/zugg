...
class TorBoxAPI:
	...
	def _post(self, endpoint, params=None, data=None, json_data=None, auth=True):
		result = None
		try:
			if auth and not self.token:
				return None
			...
			result = session.post(url, **request_kwargs).json()
		except:
			pass
		return result
	...
	def _extract_download_link(self, payload):
		if payload is None:
			return None
		...
		if isinstance(payload, list):
			for item in payload:
				result = self._extract_download_link(item)
				if result:
					return result
			return None
		if isinstance(payload, dict):
			...
			for key in ('data', 'results'):
				if key in payload:
					result = self._extract_download_link(payload.get(key))
					if result:
						return result
		return None
	...
	def _request_download(self, torrent_id, file_id=0):
		try:
			user_ip = session.get(ip_url, timeout=2.0).text
		except:
			user_ip = ''
		...
		data = self._parse_data(response)
		result = self._extract_download_link(data)
		if result:
			return result
		return self._extract_download_link(response)
	...
	def resolve_magnet(self, magnet_url, info_hash, store_to_cloud, title, season, episode):
		...
		try:
			...
			if season:
				...
				if matched:
					for item in matched:
						compare_link = seas_ep_filter(season, episode, self._file_name(item), split=True)
